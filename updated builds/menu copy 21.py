# user defined applications is stored in config.json
...
def load_config():
    ...
    try:
        with open(CONFIG_FILE, "r") as f:
            ...
            user_apps = config.get("apps", DEFAULT_CONFIG["apps"])
        ...
        # Initialize with default apps if loading fails
...
canvas = tk.Canvas(root, width=900, height=280, bg="white", highlightthickness=0, highlightbackground="lightgray")
canvas.pack()

# Add this before creating controls
...
radius = 20
canvas.create_rounded_rect(0, 0, 900, 280, radius, fill="#222222", outline="lightgray")

# Center the window on the screen
window_width = 900
window_height = 280  # Reduced height for single row
...
# Instead of using create_bottom_controls, place both datetime and volume widgets directly on canvas
...
# Add this function before creating controls
def create_app_launcher(parent):
    """Create a quick app launcher with add/remove functionality"""
    global user_apps  # Add this line
    launcher_frame = tk.Frame(parent, bg="#222222")
    apps_frame = tk.Frame(launcher_frame, bg="#222222")
    apps_frame.pack(side=tk.LEFT)
    ...
    def update_app_buttons():
        ...
        for name, path in user_apps.items():
            btn = tk.Button(apps_frame, text=name,
    ...
    control_frame.pack(side=tk.LEFT, padx=(10, 0))
...
# Create only the necessary controls
top_controls = create_top_controls(button_frame)
menu_controls = create_menu_controls(button_frame)
app_launcher = create_app_launcher(button_frame)  # Create app launcher

# Arrange the controls with minimal padding
top_controls.grid(row=0, column=0, pady=(0, 5))  # Media controls
menu_controls.grid(row=1, column=0, pady=(0, 5))  # Menu icons
app_launcher.grid(row=2, column=0, pady=(0, 5))  # App launcher buttons

# Update the canvas and window height to accommodate the new row
canvas.configure(height=320)  # Increase canvas height
root.geometry("900x320")  # Increase window height

# Update the rounded rectangle size
canvas.create_rounded_rect(0, 0, 900, 320, radius, fill="#222222", outline="lightgray")

# Update window position calculation
window_width = 900
window_height = 320  # Update height
screen_width = root.winfo_screenwidth()
screen_height = root.winfo_screenheight()
