# shortcut key optmized for working long durations
...
CURRENT_VERSION = "0.0.4" # current version of program <---------<----------<-----------------<-----------<---------------<-----------------<-----
...
def search_google():
    try:
        ...
        if query:
            # Check if query starts with @youtube
            if query.lower().startswith("@youtube"):
                # Extract YouTube search query
                youtube_query = query[len("@youtube"):].strip()
                if youtube_query:
                    encoded_query = quote(youtube_query)
                    logging.info(f"Searching YouTube for: {youtube_query}")
                    url = f"https://www.youtube.com/results?search_query={encoded_query}"
                else:  # If just "@youtube" with no query, open YouTube homepage
