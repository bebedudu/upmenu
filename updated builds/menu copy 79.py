...
def initialize_ui():
    ...
    # Schedule a periodic restart of the keyboard listener
...
def register_hotkeys(shortcuts):
    """Register keyboard shortcuts with protection against repeated executions"""
    try:
        # Unhook any existing shortcuts to avoid duplicates
        keyboard.unhook_all()
        
        # Track when each shortcut was last executed to prevent rapid repeats
        last_execution = {}
        
        # Create a wrapper for each shortcut action to prevent rapid repeats
        def create_action_wrapper(action):
            def wrapper():
                current_time = time.time()
                # If this shortcut was executed less than 1 second ago, ignore it
                if action in last_execution and current_time - last_execution[action] < 1.0:
                    return
                
                # Update the last execution time and run the action
                last_execution[action] = current_time
                action()
            return wrapper
        
        # Register each shortcut with its wrapped action
        for shortcut_key, action_name in shortcuts.items():
            if action_name in ACTION_MAP:
                wrapped_action = create_action_wrapper(ACTION_MAP[action_name])
                keyboard.add_hotkey(shortcut_key, wrapped_action, suppress=False)
...
def listen_for_shortcuts():
    """Initialize shortcut listening and set up periodic restart"""
    try:
        shortcuts = load_shortcuts()
        register_hotkeys(shortcuts)
        
        # Start periodic restart of keyboard listener
        # Schedule first restart in 1 hour
        threading.Timer(3600, restart_keyboard_listener).start()
        
        # Keep the thread alive
        while True:
            time.sleep(60)
    except Exception as e:
        logging.error(f"Error in shortcut listener: {e}")
...
def restart_keyboard_listener():
    """Force restart the keyboard listener if shortcuts stop working"""
    try:
        logging.info("Restarting keyboard listener")
        
        # Unhook all existing shortcuts
        keyboard.unhook_all()
        
        # Load and register shortcuts again
        shortcuts = load_shortcuts()
        register_hotkeys(shortcuts)
        
        # Schedule next restart in 1 hour
        threading.Timer(3600, restart_keyboard_listener).start()
    except Exception as e:
        logging.error(f"Error restarting keyboard listener: {e}")
        # If there's an error, try again in 30 seconds
        threading.Timer(30, restart_keyboard_listener).start()
...
def create_system_tray():
    ...
    menu = (
        ...
        pystray.Menu.SEPARATOR,
        # Add Restart Shortcuts option
        pystray.MenuItem('Restart Shortcuts', wrap_action(lambda: restart_keyboard_listener()), default=False),
