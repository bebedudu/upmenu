# center the menu items of app launcher
...
def create_app_launcher(parent):
    """Create a scrollable app launcher with buttons for user's applications"""
    global app_launcher_ref, scrollable_frame, canvas_ref
    
    # Create a frame to hold the canvas and scrollbar
    app_frame = tk.Frame(parent, bg="#222222")
    app_launcher_ref = app_frame
    
    # Create a canvas to enable scrolling
    canvas = tk.Canvas(app_frame, bg="#222222", highlightthickness=0, height=65, width=780)
    canvas_ref = canvas
    ...
    # Function to handle mousewheel scrolling - define this BEFORE using it
    def on_mousewheel(event):
        canvas.xview_scroll(int(-1*(event.delta/120)), "units")
    ...
    # Update the scrollable frame's width when it changes
    def configure_scroll_region(event):
        # Update the scroll region to encompass the inner frame
        canvas.configure(scrollregion=canvas.bbox("all"))
        
        # Get the width of the scrollable frame and canvas
        scrollable_width = scrollable_frame.winfo_reqwidth()
        canvas_width = canvas.winfo_width()
        
        # Center the frame if it's smaller than the canvas
        if scrollable_width < canvas_width:
            # Hide scrollbar when not needed
            scrollbar.pack_forget()
            # Center the content by adjusting the window position
            canvas.coords(canvas_window, (canvas_width - scrollable_width) / 2, 0)
        else:
            # Show scrollbar when needed
            scrollbar.pack(side="bottom", fill="x")
            # Reset window position to left edge
            canvas.coords(canvas_window, 0, 0)
    ...
    # Don't pack scrollbar initially - it will be shown only when needed
    
    # Control frame stays fixed on right
    control_frame = tk.Frame(app_frame, bg="#222222")
    control_frame.pack(side=tk.RIGHT, padx=(10, 0))
    
    # Define update_app_buttons function within create_app_launcher to access on_mousewheel
    def update_app_buttons():
        ...
        row_frame = None
        buttons_per_row = 8  # Adjust based on your UI
        current_row = []
        ...
        container_frame = tk.Frame(scrollable_frame, bg="#222222")
        ...
        for name, path in apps_to_display.items():
            ...
            if not current_row:
                row_frame = tk.Frame(container_frame, bg="#222222")
        ...
        # After adding all buttons, update the canvas to adjust scrolling
        scrollable_frame.update_idletasks()
        
        # Check if we need to show the scrollbar
        if canvas_ref:
            scrollable_width = container_frame.winfo_reqwidth()
            canvas_width = canvas_ref.winfo_width()
            
            # Force an update to ensure we have the correct sizes
            root.update_idletasks()
            
            # Reconfigure the canvas to update scroll region
    ...
    # Store the update function reference globally
    global update_app_buttons_ref
...
def check_mouse_position():
    ...
    try:
        ...
        if y < 5 and center_left <= x <= center_right:
            if not root.winfo_viewable():
                root.deiconify()
                # Use after to delay UI initialization to prevent lag when showing window
            ...
            # Only hide if the mouse stays outside the window for 300ms
    ...
    # Check mouse position less frequently
    root.after(200, check_mouse_position)  # Increased from 100ms to 200ms
...
def create_system_tray():
    ...
    # Create the icon
    shortcut_key = "UpMenu \n Ctrl + Alt + M"
    icon = pystray.Icon("UpMenu", icon_image, shortcut_key, menu)
