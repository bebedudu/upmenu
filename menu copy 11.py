# more enhanced menu UI search and speaker control 
...
search_entry = tk.Entry(search_frame, 
                       width=52,
                       font=("Arial", 12),  # Increased font size for more height
                       bg="lightgray",
                       fg="black",
...
search_entry.pack(side=tk.LEFT, padx=10, ipady=8)  # Increased ipady for more height
search_entry.bind("<Return>", on_enter)

# Create search button with icon
search_control = tk.Frame(search_frame, bg="#222222")
search_control.pack(side=tk.LEFT, padx=10)
...
search_btn.image = search_img  # Keep reference
...
def create_media_controls(parent):
    ...
    speaker_control = tk.Frame(media_frame, bg="#222222")  # New frame for speaker
...
# Add this after creating media controls
menu_controls = create_menu_controls(button_frame)
menu_controls.grid(row=2, column=0, columnspan=len(buttons)+1, pady=5)
...
def arrange_buttons(event=None):
    ...
    menu_controls.grid(row=2, column=0, columnspan=len(button_widgets), pady=5)
    
    # Then arrange other buttons starting from row 3
    for widget in button_frame.winfo_children():
        if widget not in (search_frame, media_controls, menu_controls):
            widget.grid_forget()
    ...
    # Place buttons in grid, starting from row 3 (after search, media and menu controls)
    for i, button in enumerate(button_widgets):
        row = (i // buttons_per_row) + 3  # Add 3 to start after search, media and menu controls
    ...
    for row in range(rows + 3):  # +3 for search, media and menu controls
