# show the stored application when new application is added
    ...
    try:
        if os.path.exists(CONFIG_FILE):  # Check if config file exists
            with open(CONFIG_FILE, "r") as f:
                ...
                # Load apps from config, fallback to default if "apps" key doesn't exist
                user_apps = config.get("apps", {})
                if not user_apps:  # If apps dict is empty, use defaults
...
def create_app_launcher(parent):
    ...
    def update_app_buttons():
        ...
        # Debug print to check loaded apps
    ...
    update_app_buttons()  # This will now show the loaded apps
...
# Update volume and datetime widget positions
volume_frame = tk.Frame(canvas, bg="#222222")
volume_frame.place(relx=0.02, rely=0.92, anchor="sw")  # Adjust position
...
datetime_widget.place(relx=0.98, rely=0.92, anchor="se")  # Adjust position
