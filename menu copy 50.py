# optimized the context menu in menu 
...
MOUSE_CHECK_INTERVAL = 200  # milliseconds between mouse position checks
AUTO_HIDE_DELAY = 300  # milliseconds before hiding the menu
UPDATE_INTERVAL_MEDIUM = 1000  # milliseconds for medium-priority updates
...
def list_open_windows_window():
    ...
    def show_context_menu(event):
        global selected_window_title, context_menu_open
        try:
            ...
            # Schedule a function to reset topmost after menu is closed
    ...
    context_menu.add_command(label="Activate", command=lambda: activate_window(None))
    context_menu.add_command(label="Close", command=lambda: close_window())
    context_menu.add_command(label="Minimize", command=lambda: minimize_window())
    context_menu.add_command(label="Maximize", command=lambda: maximize_window())
    
    # Add a handler for when the menu is unposted (closed)
    context_menu.bind("<Unmap>", lambda e: on_context_menu_close())
...
def check_mouse_position():
    global last_mouse_position, hide_timer, menu_hidden, context_menu_open
    ...
    try:
        ...
        if y < 5 and center_left <= x <= center_right:
            ...
            # Don't hide if a context menu is open
            if root.winfo_viewable() and not hide_timer and not context_menu_open:
                hide_timer = root.after(AUTO_HIDE_DELAY, hide_window)
    except Exception as e:
