# added the brightness control in image menu
...
canvas = tk.Canvas(root, width=900, height=240, bg="white", highlightthickness=0, highlightbackground="lightgray")
canvas.pack()
...
radius = 20
canvas.create_rounded_rect(0, 0, 900, 240, radius, fill="#222222", outline="lightgray")

# Center the window on the screen
window_width = 900
window_height = 240  # Reduced height for single row
...
# Add these function definitions before creating the controls
...
def create_menu_controls(parent):
    ...
    brightness_control = tk.Frame(menu_frame, bg="#222222")  # Add brightness control
    
    # Function to get brightness icon based on current level
    def get_brightness_icon():
    ...
    lock_btn = tk.Button(lock_control, image=images['lock'], command=lock_screen,
                         bg="#222222", activebackground="#333333", bd=0, cursor="hand2")
    brightness_btn = tk.Button(brightness_control, image=images[get_brightness_icon()], command=cycle_brightness,
                         bg="#222222", activebackground="#333333", bd=0, cursor="hand2")
    ...
    lock_label = tk.Label(lock_control, text="Lock Screen", fg="lightgray",
                          bg="#222222", font=("Arial", 10), cursor="hand2")
    brightness_label = tk.Label(brightness_control, text="Brightness", fg="lightgray",
                          bg="#222222", font=("Arial", 10), cursor="hand2")
    ...
    brightness_btn.image = images[get_brightness_icon()]
    ...
    # Function to update brightness icon
    def update_brightness_icon():
        brightness_btn.config(image=images[get_brightness_icon()])
        brightness_btn.image = images[get_brightness_icon()]
        menu_frame.after(1000, update_brightness_icon)
    
    # Start updating brightness icon
...
menu_controls.grid(row=1, column=0, pady=5)

# Modify arrange_buttons function to be simpler
def arrange_buttons(event=None):
    # Just update the frame size
